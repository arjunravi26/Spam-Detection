...
class DataTransformation:
    ...
    def preprocess_data(self):
        try:
            ...
            train_embeddings = [self.get_weighted_word2vec(doc,idx) for idx,doc in enumerate(train_words)]
            test_embeddings = [self.get_weighted_word2vec(doc,idx) for idx,doc in enumerate(test_words)]
            validation_embeddings = [self.get_weighted_word2vec(doc,idx) for idx,doc in enumerate(validation_words)]
    ...
    def preprocess_new_data(self, data):
        try:
            ...
            transformed_data = [self.test_get_weighted_word2vec(documents,tfidf_vectorizer,tfidf_vectorizer.transform([documents]))]
